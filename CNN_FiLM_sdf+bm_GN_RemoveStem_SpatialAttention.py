# =============================================================
# CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.py  ——  边界条件 → 3D 温度场
...
class SpatialAttention(nn.Module):
    """CBAM-style Spatial Attention for 3D: concat(mean,max)->conv->sigmoid"""
...
def create_cnn3d_from_params(params: Dict[str, Any], input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    ...
    class CNN3D_FiLM(nn.Module):
        def __init__(self):
            ...
            # full-res channels kept small to avoid OOM
            c0 = max(8, base_ch // 2)  # you can tune this (e.g., 8/12), but keep it small

            # Full-res "pre-encoder" block: (7 -> c0) at resolution /1
            self.enc0_full = FiLMResidualBlock(7, c0, dropout_p=dropout_p)  # res: /1
            ...
            self.enc0 = FiLMResidualBlock(c0, base_ch, dropout_p=dropout_p)  # res: /2
            ...
            # --- Spatial Attention for skip tensors (apply BEFORE concat) ---
            self.sa_x1 = SpatialAttention()  # skip x1 (/4)
            self.sa_x0 = SpatialAttention()  # skip x0 (/2)
            self.sa_full = SpatialAttention()  # skip x_full_skip (/1)
            ...
            # enc0_full output channels = c0 (defined earlier)
            self.up0_conv = FiLMResidualBlock(base_ch + c0, c0, dropout_p=dropout_p)  # /2 -> /1

            # Final prediction at full resolution
            self.final_conv = nn.Conv3d(c0, 1, kernel_size=1)

            # ===== FiLM 注入点通道列表（顺序必须与 forward 的 gi 使用一致）=====
            # 0) enc0_full (full-res)
            # 1) enc0 (base_ch)
            # 2) enc1 (base_ch*2)
            # 3) enc2 (base_ch*4)
            ...
            ch_list = [
                c0,  # enc0_full (full-res)
                base_ch,  # enc0 (/2)
                base_ch * 2,  # enc1 (/4)
                base_ch * 4,  # enc2 (/8)
                base_ch * 4,  # bottleneck (/8)
                base_ch * 2,  # up2 (/4)
                base_ch,  # up1 (/2)
                c0,  # up0 (full-res)   <-- NEW
            ]
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            x_full_skip = self.enc0_full(vox, gammas[gi], betas[gi]);
            gi += 1  # /1
            ...
            # ---- Encoder ----
            x0 = self.enc0(x, gammas[gi], betas[gi]);
            gi += 1  # /2
            x1 = self.pool1(x0)
            x1 = self.enc1(x1, gammas[gi], betas[gi]);
            gi += 1  # /4
            x2 = self.pool2(x1)
            x2 = self.enc2(x2, gammas[gi], betas[gi]);
            gi += 1  # /8
            ...
            # ✅ apply SA to skip x1 BEFORE concat
            x1_att = self.sa_x1(x1)
            x_cat2 = torch.cat([x_up2, x1_att], dim=1)
            x_dec2 = self.up2_conv(x_cat2, gammas[gi], betas[gi]);
            gi += 1

            x_up1 = F.interpolate(x_dec2, size=x0.shape[2:], mode="trilinear", align_corners=False)  # /4 -> /2

            # ✅ apply SA to skip x0 BEFORE concat
            x0_att = self.sa_x0(x0)
            x_cat1 = torch.cat([x_up1, x0_att], dim=1)
            x_dec1 = self.up1_conv(x_cat1, gammas[gi], betas[gi]);
            gi += 1
            ...
            # ✅ apply SA to skip x_full_skip BEFORE concat
            x_full_att = self.sa_full(x_full_skip)
            x_cat0 = torch.cat([x_up0, x_full_att], dim=1)
            x0_full = self.up0_conv(x_cat0, gammas[gi], betas[gi]);
            gi += 1
...
def train_final(data: Dict[str, Any], best_params: Dict[str, Any], *, seed: int, out_prefix: str):
    ...
    save_common = {
        ...
        "model_type": "CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention",
...
def parse_args():
    ...
    ap.add_argument("--params_json", type=str, default="best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.json",
                    help="Path to save/load best_params JSON.")
    ap.add_argument("--out_prefix", type=str, default="CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention",
...
if __name__ == "__main__":
    main()
# CUDA_VISIBLE_DEVICES=1,4,6 python CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.py --run_mode search --n_trials 20 --params_json best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.json
# CUDA_VISIBLE_DEVICES=2,4 python CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.py --run_mode train --seed 47 --params_json best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention.json --out_prefix CNN_FiLM_sdf+bm_GN_RemoveStem_SpatialAttention
