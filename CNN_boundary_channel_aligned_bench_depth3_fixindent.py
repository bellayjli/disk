# =============================================================
# CNN.py  ——  边界条件 → 3D 温度场
# 3D UNet (depth=3) + Optuna + Mask + 梯度损失 + 小批量训练（避免 OOM）
# 输入（13 通道体素）：
#   - 7 通道体素来自：cnn_input_channels_no_normals.csv 的后 7 列
#   - 6 个边界条件通道来自：boundary_condition.csv（每个样本复制到整个体素网格）
...
def create_cnn3d_from_trial(trial: optuna.trial.Trial, input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    """
    无 FiLM（depth=3）：
    - 固定 7 通道体素场（来自 cnn_input_channels_no_normals.csv）
      体素通道：[C0, C1, C2, C3, C4, C5, sdf]
    - 边界条件 bc (R^6) 直接复制成 6 个通道，拼到体素上：bc_grid (B,6,nx,ny,nz)
    - 最终输入 CNN 的通道数：7 + 6 = 13
    """
    ...
    class CNN3D_NoFiLM(nn.Module):
        def __init__(self):
            ...
            # Encoder (depth=3)
            self.enc0 = ConvBlock(base_ch, base_ch, dropout_p=dropout_p)            # /2
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            # 全分辨率拼接：decoder 特征 + voxel(7) + bc(6)
...
# （可选一致性检查）如果你期望 CSV 的后 7 列也正好是这 7 个通道，可检查：
...
# ✅ 监督/门控 mask：必须来自 C0 列（inside_mask），与 voxel 通道选择解耦
...
torch.save(
    ...
    "best_CNN_boundary_channel_aligned_bench.pth",
)
print("模型已保存为 best_CNN_boundary_channel_aligned_bench.pth")
# CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 python "CNN_boundary_channel_aligned_bench.py"
