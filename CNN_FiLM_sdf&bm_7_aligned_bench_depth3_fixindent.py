...
# 3D UNet (depth=3) + Optuna + Mask + 梯度损失
#
...
def create_cnn3d_from_trial(trial: optuna.trial.Trial, input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    ...
    depth = 3
    base_ch = trial.suggest_categorical("base_ch", [12, 16, 24, 32])
    dropout_p = trial.suggest_float("dropout_p", 0.0, 0.3)

    # ===== FiLM 搜索范围（扩展）=====
    film_mult = trial.suggest_categorical("film_mult", [1, 2, 4])
    film_hidden = int(base_ch * film_mult)

    # 输出缩放，避免早期 gamma/beta 幅度过大导致训练不稳
    film_scale = trial.suggest_float("film_scale", 0.03, 2.0, log=True)
    ...
    class CNN3D_FiLM(nn.Module):
        def __init__(self):
            ...
            # Encoder (depth=3)
            self.enc0 = FiLMResidualBlock(base_ch, base_ch, dropout_p=dropout_p)          # res: /2
            self.pool1 = nn.MaxPool3d(kernel_size=2, stride=2)                            # res: /4
            self.enc1 = FiLMResidualBlock(base_ch, base_ch * 2, dropout_p=dropout_p)      # res: /4
            self.pool2 = nn.MaxPool3d(kernel_size=2, stride=2)                            # res: /8
            self.enc2 = FiLMResidualBlock(base_ch * 2, base_ch * 4, dropout_p=dropout_p)  # res: /8
            ...
            # Decoder
            self.up2_conv = FiLMResidualBlock(bottleneck_ch + base_ch * 2, base_ch * 2, dropout_p=dropout_p)  # /8 -> /4
            self.up1_conv = FiLMResidualBlock(base_ch * 2 + base_ch, base_ch, dropout_p=dropout_p)            # /4 -> /2
            ...
            # 2) enc1 (base_ch*2)
            # 3) enc2 (base_ch*4)
            # 4) bottleneck (base_ch*4)
            # 5) up2 (base_ch*2)
            # 6) up1 (base_ch)
            # 7) geom_block (base_ch)
            ch_list = [
                base_ch,        # stem
                base_ch,        # enc0
                base_ch * 2,    # enc1
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            # ---- Encoder ----
            x0 = self.enc0(x, gammas[gi], betas[gi]); gi += 1                 # /2
            x1 = self.pool1(x0)
            x1 = self.enc1(x1, gammas[gi], betas[gi]); gi += 1                # /4
            x2 = self.pool2(x1)
            x2 = self.enc2(x2, gammas[gi], betas[gi]); gi += 1                # /8

            # ---- Bottleneck ----
            xb = self.bottleneck(x2, gammas[gi], betas[gi]); gi += 1           # /8

            # ---- Decoder ----
            x_up2 = F.interpolate(xb, size=x1.shape[2:], mode="trilinear", align_corners=False)  # /8 -> /4
            x_cat2 = torch.cat([x_up2, x1], dim=1)
            x_dec2 = self.up2_conv(x_cat2, gammas[gi], betas[gi]); gi += 1     # /4

            x_up1 = F.interpolate(x_dec2, size=x0.shape[2:], mode="trilinear", align_corners=False)  # /4 -> /2
            x_cat1 = torch.cat([x_up1, x0], dim=1)
            x_dec1 = self.up1_conv(x_cat1, gammas[gi], betas[gi]); gi += 1     # /2
...
def objective(trial):
    ...
    grad_weight = trial.suggest_float("grad_weight", 0.015, 0.2, log=True)
...
# =============================================================
# Training efficiency / memory benchmark
# - Step time (ms/iter): CUDA event elapsed time over forward+backward+step
# - Peak VRAM (GB): max allocated + max reserved
#
# FP16 / AMP：
# - 设置环境变量 USE_AMP=1 启用 autocast+GradScaler（默认关闭，保持与你现在一致）
# - 统计使用 torch.cuda.reset_peak_memory_stats() 在 warmup 后清零，避免 warmup 污染峰值
# =============================================================
USE_AMP = (device.type == "cuda" and int(os.environ.get("USE_AMP", "0")) == 1)
scaler = torch.cuda.amp.GradScaler(enabled=USE_AMP)

REPORT_TRAIN_BENCH = True
BENCH_EPOCH = 0
BENCH_WARMUP = 5
BENCH_STEPS = 30

_bench_times_ms: list[float] = []
_bench_peak_alloc_bytes = 0
_bench_peak_reserved_bytes = 0

for epoch in range(num_epochs_b):
    model_b.train()
    running_loss = 0.0

    for step_idx, (xb, yb, mb) in enumerate(trainval_loader):
        do_bench = (REPORT_TRAIN_BENCH and device.type == "cuda" and epoch == BENCH_EPOCH)

        if do_bench and step_idx == BENCH_WARMUP:
        ...
        if do_bench:
            ...
            _bench_times_ms.append(ms)

            _bench_peak_alloc_bytes = max(_bench_peak_alloc_bytes, int(torch.cuda.max_memory_allocated()))
            _bench_peak_reserved_bytes = max(_bench_peak_reserved_bytes, int(torch.cuda.max_memory_reserved()))

            if len(_bench_times_ms) >= BENCH_STEPS:
                steady = _bench_times_ms[BENCH_WARMUP:] if len(_bench_times_ms) > BENCH_WARMUP else _bench_times_ms
                avg_ms = float(np.mean(steady)) if len(steady) else float(np.mean(_bench_times_ms))
                peak_alloc_gb = _bench_peak_alloc_bytes / (1024 ** 3)
                peak_reserved_gb = _bench_peak_reserved_bytes / (1024 ** 3)
                print(f"[bench][train] AMP(fp16)={USE_AMP}  step: {avg_ms:.3f} ms/iter (avg over {len(steady)} iters)")
                print(f"[bench][train] Peak VRAM allocated: {peak_alloc_gb:.3f} GB")
                print(f"[bench][train] Peak VRAM reserved : {peak_reserved_gb:.3f} GB")
                REPORT_TRAIN_BENCH = False
...
torch.save(
    ...
    "best_CNN_FiLM_sdf&bm_7_aligned_bench.pth",
)
print("模型已保存为 best_CNN_FiLM_sdf&bm_7_aligned_bench.pth")
# CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 python "CNN_FiLM_sdf&bm_7_aligned_bench_depth3_fixindent.py"
