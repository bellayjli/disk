...
def create_cnn3d_from_params(params: Dict[str, Any], input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    ...
    class CNN3D_FiLM_BottleneckOnly(nn.Module):
        def __init__(self):
            ...
            # full-res pre-encoder block: 7 -> c0 (NO FiLM now)
            self.enc0_full = ConvBlock(7, c0, dropout_p=dropout_p)

            self.pool0 = nn.MaxPool3d(2, 2)

            # encoder blocks (NO FiLM)
            self.enc0 = ConvBlock(c0, base_ch, dropout_p=dropout_p)
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            # ---- Bottleneck (FiLM ONLY HERE) ----
            gamma, beta = self.film(bc)  # (B,C,1,1,1)
...
def train_final(data: Dict[str, Any], best_params: Dict[str, Any], *, seed: int, out_prefix: str):
    ...
    save_common = {
        ...
        "model_type": "CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck",
...
def parse_args():
    ...
    ap.add_argument("--params_json", type=str, default="best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck.json",
                    help="Path to save/load best_params JSON.")
    ap.add_argument("--out_prefix", type=str, default="CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck",
...
if __name__ == "__main__":
    main()
# CUDA_VISIBLE_DEVICES=1,4,6 python CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck.py --run_mode search --n_trials 20 --params_json best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck.json
# CUDA_VISIBLE_DEVICES=2,4 python CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck.py --run_mode train --seed 47 --params_json best_params_CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck.json --out_prefix CNN_FiLM_sdf+bm_GN_RemoveStem_Bottleneck