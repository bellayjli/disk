...
class ConvResidualBlock(nn.Module):
    ...
    （对齐 FiLMResidualBlock 的写法：conv1/conv2/norm1/norm2/act/drop/residual_proj）
    """
...
def create_cnn3d_from_params(params: Dict[str, Any], input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    ...
    class CNN3D_NoFiLM(nn.Module):
        def __init__(self):
            ...
            self.enc0 = ConvResidualBlock(c0, base_ch, dropout_p=dropout_p)  # res: /2
            self.pool1 = nn.MaxPool3d(kernel_size=2, stride=2)  # res: /4
            self.enc1 = ConvResidualBlock(base_ch, base_ch * 2, dropout_p=dropout_p)  # res: /4
            self.pool2 = nn.MaxPool3d(kernel_size=2, stride=2)  # res: /8
            self.enc2 = ConvResidualBlock(base_ch * 2, base_ch * 4, dropout_p=dropout_p)  # res: /8
            ...
            self.up0_conv = ConvResidualBlock(base_ch + c0, c0, dropout_p=dropout_p)  # /2 -> /1
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            x_full_skip = self.enc0_full(x_in_full)  # (B,c0,nx,ny,nz)   /1
            ...
            x_full = self.up0_conv(x_cat0)  # (B, c0, nx, ny, nz)

            x_out = self.final_conv(x_full)  # (B,1,nx,ny,nz)
...
def parse_args():
    ...
    ap.add_argument("--params_json", type=str, default="best_params_concat_unet_voxel7.json",
                    help="Path to save/load best_params JSON.")
    ap.add_argument("--out_prefix", type=str, default="CNN_Concat_sdf+bm_GN_RemoveStem",
...
if __name__ == "__main__":
    main()
# CUDA_VISIBLE_DEVICES=2,3,4 python CNN_Concat_sdf+bm_GN_RemoveStem.py --run_mode search --n_trials 20 --params_json best_params_CNN_Concat_sdf+bm_GN_RemoveStem.json
# CUDA_VISIBLE_DEVICES=1,2,4 python CNN_Concat_sdf+bm_GN_RemoveStem.py --run_mode train --seed 44 --params_json best_params_CNN_Concat_sdf+bm_GN_RemoveStem.json --out_prefix CNN_Concat_sdf+bm_GN_RemoveStem