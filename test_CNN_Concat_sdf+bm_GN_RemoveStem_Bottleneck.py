...
# - 模型结构：严格对齐训练脚本 CNN3D_NoFiLM(depth=3)：
# 入口输入通道=7（voxel）
# bottleneck 拼接 bc：在 /8 分辨率处通道 +6
# full-res 仍然只用 voxel（并保留 full-res skip）
...
def build_model_from_ckpt(ckpt: dict) -> nn.Module:
    ...
    if depth != 3:
        print(f"[warn] ckpt.depth={depth}，但当前测试脚本将按 depth=3 架构构建模型以对齐训练脚本。")
    ...
    class CNN3D_NoFiLM(nn.Module):
        def __init__(self):
            ...
            # full-res light block: 7(voxel) -> c0   （bc 不在入口拼接）
            self.enc0_full = ConvResidualBlock(7, c0, dropout_p=dropout_p)  # /1
            ...
            # ✅ bc 只在 bottleneck 处拼：in_ch = bottleneck_ch + 6
            self.bottleneck = ConvResidualBlock(bottleneck_ch + 6, bottleneck_ch, dropout_p=dropout_p)
            ...
            # ✅ 新的最后一层：/2 -> /1 后 concat full-res skip
            self.up0_conv = ConvResidualBlock(base_ch + c0, c0, dropout_p=dropout_p)  # /1
        ...
        def forward(self, bc: torch.Tensor) -> torch.Tensor:
            ...
            # ---- full-res light block (only voxel) ----
            x_full_skip = self.enc0_full(vox)  # (B,c0,nx,ny,nz)
            # ---- downsample to /2 ----
            x = self.pool0(x_full_skip)  # (B,c0,/2)

            # ---- encoder (same pattern as before) ----
            ...
            # ✅ bc -> /8 尺寸网格，直接 expand 到 x2 的空间尺寸
            bc_grid_8 = bc.view(B, 6, 1, 1, 1).expand(B, 6, x2.shape[2], x2.shape[3], x2.shape[4])  # (B,6,/8)
            ...
            x_dec = self.out_proj(x_dec1)  # (B,base_ch,/2)

            # ---- ✅ final: /2 -> /1 + concat full-res skip ----
            x_up0 = F.interpolate(x_dec, size=x_full_skip.shape[2:], mode="trilinear", align_corners=False)  # /1
            x0_full = self.up0_conv(torch.cat([x_up0, x_full_skip], dim=1))  # (B,c0,/1)
...
CKPT_PATH = "CNN_Concat_sdf+bm_GN_RemoveStem_Bottleneck_seed43_lastTrain.pth"
...
# ===================== 预测保存设置 =====================
SAVE_DIR = "test_outputs_CNN_Concat_sdf+bm_GN_RemoveStem_Bottleneck"
...
# CUDA_VISIBLE_DEVICES=1,3,4,5,6 python "test_CNN_Concat_sdf+bm_GN_RemoveStem_Bottleneck.py"
