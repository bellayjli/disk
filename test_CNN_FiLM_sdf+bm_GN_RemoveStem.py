...
def build_model_from_ckpt(ckpt: dict) -> nn.Module:
    ...
    allowed_prefixes = {
        "FiLM_A_voxel7_no_stem_pooldown_fullres_light"
    }
    ...
    ny = int(ckpt["ny"])
    nz = int(ckpt["nz"])
    # 训练脚本固定 depth=3，这里从 ckpt 读取并强约束
    ...
    class CNN3D_FiLM(nn.Module):
        def __init__(self):
            ...
            # 0) enc0_full (full-res), 1) enc0, 2) enc1, 3) enc2, 4) bottleneck, 5) up2, 6) up1, 7) up0(c0)
            ch_list = [
...
# CUDA_VISIBLE_DEVICES=2,4 python "test_CNN_FiLM_sdf+bm_GN_RemoveStem.py"
