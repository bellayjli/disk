...
def create_cnn3d_from_trial(trial: optuna.trial.Trial, input_dim: int, nx: int, ny: int, nz: int) -> nn.Module:
    """
    无 FiLM：
    - 固定 7 通道体素场（来自 cnn_input_channels_no_normals.csv 的后 7 列）
...
# =============================================================
# Training efficiency / memory benchmark (reported during real training steps)
# - Peak VRAM training (GB): torch.cuda.max_memory_allocated()
# - Train step time (ms/iter): CUDA event elapsed time over forward+backward+step
# Notes:
# - Measured on Stage B (train+val) epoch 0, first BENCH_STEPS iterations (after BENCH_WARMUP).
# - This adds minimal overhead and does NOT change training logic.
# =============================================================
...
for epoch in range(num_epochs_b):
    ...
    for xb, yb, mb in trainval_loader:
        ...
        loss.backward()
        optimizer_b.step()
        # ---- benchmark: end timing + peak memory ----
        if do_bench:
            end_evt.record()
            ...
            # Reset peak stats right after warmup to isolate steady-state
            if len(_bench_times_ms) == BENCH_WARMUP:
                torch.cuda.reset_peak_memory_stats()
                _bench_peak_mem_bytes = 0

            _bench_peak_mem_bytes = max(_bench_peak_mem_bytes, int(torch.cuda.max_memory_allocated()))

            # Print once when enough steps collected
            if len(_bench_times_ms) >= BENCH_STEPS:
                steady = _bench_times_ms[BENCH_WARMUP:]
                avg_ms = float(np.mean(steady)) if len(steady) > 0 else float(np.mean(_bench_times_ms))
                peak_gb = _bench_peak_mem_bytes / (1024 ** 3)
                print(f"[bench][train] Train step time: {avg_ms:.3f} ms/iter (avg over {len(steady)} iters)")
                print(f"[bench][train] Peak VRAM training: {peak_gb:.3f} GB (max allocated)")
